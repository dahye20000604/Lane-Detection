...
while cap.isOpened():
    ...
    for i in range(0,int(len(point)//2)):
        Xl.append(point[i][0][0])
        Yl.append(point[i][0][1])
        Xr.append(point[i][1][0])
        Yr.append(point[i][1][1])
    print(Xl,Yl,Xr,Yr)
    ...
    for i in range(0, 200):
        xr = int((i - zr[1]) / zr[0]);
        xl = int((i - zl[1]) / zl[0]);
        cv2.line(img, (xr, i), (xr, i), (0, 0, 0), 5)
        if abs(xr-xl)<=1:
            print(xr,i)
            corners=[point[0][0],point[0][1],[xr,i],[xl,i]]
            cv2.line(img,(xr,i),(xr,i),(0,0,0),5)
            cv2.fillPoly(copy_img, np.int32([corners]), (255, 255, 255))
    image_new = cv2.addWeighted(copy_img, 0.4, frame1.copy(), 0.6, 0)
