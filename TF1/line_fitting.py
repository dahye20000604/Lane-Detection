...
while cap.isOpened():
    ...
    right_list=[]
    l=0; r=0
    for i in range(imH - 5, 200, -5):
        for k in range(imH - 10, 200, -5):
            l = 0;
            r = 0
            if left[k]:
                for j in range(left[k] - 5, left[k] + 5):
                    if edge[i, j] == 255:
                        l = (j, i)
                        break
                if l == 0:
                ...
                for j in range(c, c - 200, -1):
                    if edge[i, j] == 255:
                        ...
                        break
            if right[k]:
                for j in range(right[k] - 3, right[k] + 3):
                    if edge[i, j] == 255:
                        r = (j, i)
                        break
                if r == 0:
    ...
    for i in range(len(point_l)-1):
        cv2.line(img,point_l[i],point_l[i+1],(0,0,0),3)
    for i in range(len(point_r)-1):
        cv2.line(img,point_r[i],point_r[i+1],(0,0,0),3)
    cv2.imshow('img2', img)
    '''
    img2=frame1.copy()
    ...
    try:
        ...
        h=100
        while 1:
            xl = point[0][0][0] + (point[0][0][0] - point[1][0][0]) / (point[1][0][1] - point[0][0][1]) * (point[0][0][1] - h)
            xr = point[0][1][0] - (point[1][1][0] - point[0][1][0]) / (point[1][1][1] - point[0][1][1]) * (point[0][1][1] - h)
            if xl<=xr:
                point.insert(0, [(xl, h), (xr, h)])
                break
            else:
                h+=1
        for i in range(len(point)-1):
            ...
            for j in range(point[i][0][1]+1,point[i+1][0][1]):
                ...
                w_list[j]=right[j]-left[j]
                if right[j]-left[j]<=0:
                    point[0]=[(left[j],j),(right[j],j)]
                    print(point[0])
                    break
        print(w_list)
