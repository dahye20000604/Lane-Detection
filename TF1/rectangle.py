...
out = cv2.VideoWriter('output_s.avi', cv2.VideoWriter_fourcc(*'MJPG'), 30.0, (int(imW), int(imH)))
# right point
right = [0] * imH
# left point
left = [0] * imH
while cap.isOpened():
    ...
    for i in range(imH - 10, 200, -5):
        ...
        if r and l:
            if r[0] - l[0] < w and r[0] - l[0] > 0:
                cv2.line(frame1,(r[0],i),(r[0],i),(255,255,0),2)
                cv2.line(frame1, (l[0], i), (l[0], i), (255, 0, 255), 2)
                right[i] = r[0]
                left[i] = l[0]
                w = r[0] - l[0]
                width[i]=w
    ...
    left[-1] = xl
    print(first, left[first], second, left[second], imH-1, xl)
    right[-1] = xr
    point = []
    for i in range(imH):
    ...
    left[100] = xl
    right[100] = xr
    point.insert(0, [(left[100], 100), (right[100], 100)])
    print(xl, xr)
    ...
    for i in range(imH):
        if left[i] and right[i]:
            for j in range(i+1,imH):
                if left[j] and right[j]:
                    corners = [(left[i], i), (left[j], j), (right[j], j), (right[i], i)]
                    cv2.fillPoly(copy_img, np.int32([corners]), (255,255,255))
                    break
    image_new = cv2.addWeighted(copy_img, 0.4, img, 0.6, 0)

    # Draw rectangle
    copy_img = frame1.copy()
    for i in range(0,imH,10):
        if left[i] and right[i]:
            for j in range(i + 10, imH):
                if left[j] and right[j]:
                    corners = [(left[i], i), (left[j], j), (right[j], j), (right[i], i)]
                    cv2.fillPoly(copy_img, np.int32([corners]), (255, 255, 255))
                    break
    ...
    cv2.putText(frame, 'FPS: {0:.2f}'.format(frame_rate_calc), (30, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2,
                cv2.LINE_AA)
    #cv2.imshow('first : lane detection', image_new)
