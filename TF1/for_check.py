...
while cap.isOpened():
    ...
    for i in range(imH):
        if left[i]:
            cv2.line(img,(left[i],i),(left[i],i),(255,0,0),5)
            left_list.append((left[i], i))
        if right[i]:
            cv2.line(img,(left[i],i),(left[i],i),(255,0,0),5)
            right_list.append((right[i], i))
    #cv2.imshow('find point',img)
    #추세선 그리기
    Xl=[];Yl=[];Xr=[];Yr=[]
    for k in range(len(left_list)):
        Xl.append(left_list[0][0])
        Yl.append(left_list[0][1])
    for k in range(len(right_list)):
        Xr.append(left_list[0][0])
        Yr.append(left_list[0][1])
    try:
        zr = np.polyfit(Xr, Yr, 1); zl = np.polyfit(Xl, Yl, 1)
        for i in range(imH-1-k,imH-k-50,-1):
            xr1 = int((i-zr[1])/zr[0]); xr2 = int((i-1 - zr[1]) / zr[0])
            #cv2.line(img_fit,(xr1,i),(xr2,i-1),(255,255,255),2)
            xl1 = int((i - zl[1]) / zl[0]); xl2 = int((i - 1 - zl[1]) / zl[0])
            #cv2.line(img_fit, (xl1, i), (xl2, i - 1), (255, 255, 255), 2)
            if abs(right[i]-xr1)>3:
                right[i]=0
            if abs(right[i-1] - xr2) > 3:
                right[i-1] = 0
            if abs(left[i]-xl1)>3:
                left[i]=0
            if abs(left[i]-xl2)>3:
        ...
        try:
            ...
            for i in range(imH-1-k,imH-k-50,-1):
                ...
                if abs(left[i]-xl2)>3:
                    left[i-1]=0
                #right[i]=xr1; right[i-1]=xr2
                #left[i] = xl1; left[i-1] = xl2
        except:
            pass
    point_l=[]
    point_r=[]
    for i in range(0,imH):
        if left[i]:
            point_l.append((left[i],i))
        if right[i]:
            point_r.append((right[i],i))
    for i in range(len(point_l)-1):
        cv2.line(img_fit,point_l[i],point_l[i+1],(255,255,255),2)
    for i in range(len(point_r)-1):
        cv2.line(img_fit,point_r[i],point_r[i+1],(255,255,255),2)





    cv2.imshow('connect point', img)
    ...
    for i in range(imH - 10, 180, -1):
        ...
        if r and l:
            if r[0] - l[0] < w and r[0] - l[0] > 0:
                if left[i] and abs(left[i]-l[0])>5:
                    continue
                elif right[i] and abs(right[i]-r[0])>5:
                    continue
    ...
    for i in range(imH):
        if cur_left[i]:
            ...
            while 1:
                if cur_left[j]:
                    cv2.line(img,(int(cur_left[i]),i),(int(cur_left[j]),j),(0,255,0),3)
                    break
                if j==imH-1:
                    break
                j+=1
        if cur_right[i]:
            ...
            while 1:
                if cur_right[j]:
                    cv2.line(img,(int(cur_right[i]),i),(int(cur_right[j]),j),(0,255,0),3)
                    break
                if j==imH-1:
                    break
                j+=1
    ...
    for i in range(len(left_list)-1):
        if abs(left_list[i][1]-left_list[i+1][1])>20:
            j=int((left_list[i+1][1]+left_list[i][1])//2)
            while 1:
                if left[j]:
                    left_list.insert(i+1,(left[j],j))
                    break
                elif j==i:
                    break
                else:
